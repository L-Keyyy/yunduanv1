...
def resolve_dimension_value(primary_value: float, fallback_value: float, default_value: int) -> int:
    if primary_value == primary_value:
        return round(primary_value)
    if fallback_value == fallback_value:
        return round(fallback_value * 10)
    return default_value


def parse_dimension_triplet(value: Any) -> Optional[Dict[str, float]]:
...
def find_triplet_dimensions(
    ...
    *,
    ...
    for item in characteristics:
        ...
        if not name:
            continue
        is_dimension_like = bool(re.search(r"(размер|габарит|dimensions|dimension)", name))
        is_package_like = bool(re.search(r"(упаков|package|shipping)", name))
        if not is_dimension_like:
            continue
        if package_only and not is_package_like:
            continue

        raw_value = normalize_text(item.get("valueText")) or ", ".join(item.get("values") or [])
        triplet = parse_dimension_triplet(raw_value)
        if not triplet:
            continue

        multiplier = 10 if re.search(r"(^|[^a-zа-я])(см|cm)([^a-zа-я]|$)", name) else 1
        return {
            "height": round(triplet["height"] * multiplier),
            "width": round(triplet["width"] * multiplier),
            "depth": round(triplet["depth"] * multiplier),
...
def extract_dimensions(characteristics: List[Dict[str, Any]]) -> Dict[str, Any]:
    height_mm = parse_numeric_value(
        get_first_char_value(characteristics, ["Высота упаковки, мм", "Высота, мм"])
    )
    width_mm = parse_numeric_value(
        get_first_char_value(characteristics, ["Ширина упаковки, мм", "Ширина, мм"])
    )
    depth_mm = parse_numeric_value(
        get_first_char_value(
            characteristics,
            ["Длина упаковки, мм", "Глубина упаковки, мм", "Длина, мм", "Глубина, мм"],
        )
    )
    height_cm = parse_numeric_value(get_char_value(characteristics, "Высота, см"))
    width_cm = parse_numeric_value(get_char_value(characteristics, "Ширина, см"))
    depth_cm = parse_numeric_value(
        get_first_char_value(characteristics, ["Глубина, см", "Длина, см"])
    )
    found_count = sum(
        1
        for value in (
            height_mm == height_mm or height_cm == height_cm,
            width_mm == width_mm or width_cm == width_cm,
            depth_mm == depth_mm or depth_cm == depth_cm,
        )
        if value
    )
    return {
        "height": resolve_dimension_value(height_mm, height_cm, DEFAULT_HEIGHT_MM),
        "width": resolve_dimension_value(width_mm, width_cm, DEFAULT_WIDTH_MM),
        "depth": resolve_dimension_value(depth_mm, depth_cm, DEFAULT_DEPTH_MM),
        "dimension_unit": "mm",
        "source": "default" if found_count == 0 else "json" if found_count == 3 else "mixed",
    }


def extract_preferred_dimensions(characteristics: List[Dict[str, Any]]) -> Dict[str, Any]:
    package_triplet = find_triplet_dimensions(characteristics, package_only=True)
    generic_triplet = None if package_triplet else find_triplet_dimensions(characteristics)
    base = extract_dimensions(characteristics)
    chosen = package_triplet or generic_triplet
    if not chosen:
        return base
    return {
        **base,
...
def extract_weight(characteristics: List[Dict[str, Any]], scraped_data: Dict[str, Any]) -> Dict[str, Any]:
    package_weight_grams = parse_numeric_value(
        get_first_char_value(characteristics, ["Вес с упаковкой, г", "Вес товара с упаковкой, г"])
    )
    if package_weight_grams == package_weight_grams:
        return {"weight": round(package_weight_grams), "weight_unit": "g", "source": "json-package-g"}

    package_weight_kg = parse_numeric_value(
        get_first_char_value(characteristics, ["Вес с упаковкой, кг", "Вес товара с упаковкой, кг"])
    )
    if package_weight_kg == package_weight_kg:
        return {"weight": round(package_weight_kg * 1000), "weight_unit": "g", "source": "json-package-kg"}

    checkout_weight_kg = scraped_data.get("packageWeight", {}).get("weightKg")
    try:
        checkout_weight = float(checkout_weight_kg)
    except Exception:
        checkout_weight = float("nan")
    if checkout_weight == checkout_weight and checkout_weight > 0:
        return {"weight": round(checkout_weight * 1000), "weight_unit": "g", "source": "checkout"}

    weight_grams = parse_numeric_value(get_first_char_value(characteristics, ["Вес, г", "Вес товара, г"]))
    if weight_grams == weight_grams:
        return {"weight": round(weight_grams), "weight_unit": "g", "source": "json-g"}

    weight_kg = parse_numeric_value(get_first_char_value(characteristics, ["Вес, кг", "Вес товара, кг"]))
    if weight_kg == weight_kg:
        return {"weight": round(weight_kg * 1000), "weight_unit": "g", "source": "json-kg"}
    ...
    for char in scraped_chars:
        ...
        for raw_value in values:
            ...
            if dictionary_id > 0:
                ...
                matched_value = find_dict_value(dict_values, text_value)
                if matched_value:
                    ozon_values.append(
                        {
...
async def build_upload_item(
    *,
    ...
    async def get_dict_values(attribute_id: int) -> List[Dict[str, Any]]:
        return await get_cached_attribute_values_data(
            client_id,
            api_key,
            attribute_id=attribute_id,
            description_category_id=description_category_id,
    ...
    color_attr = next(
        (
            attr
            for attr in ozon_attributes
            if "цвет" in normalize_attribute_key(attr.get("name"))
            or normalize_attribute_key(attr.get("name")) == "color"
        ),
        None,
    )
    if color_attr and not has_meaningful_attribute(
        next((item for item in formatted_attributes if int(item.get("id") or 0) == int(color_attr.get("id") or 0)), None)
    ):
        color_value = normalize_color_value(
        ...
        if color_value:
            upsert_attribute(
                formatted_attributes,
                color_attr,
                await build_text_attribute_values(color_attr, color_value, get_dict_values),
            )
