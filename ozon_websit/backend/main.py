    ...
    for source_payload in source_payloads:
        try:
            ...
            if isinstance(item, dict):
                ...
                build_errors.append("build_upload_item returned invalid item payload")
...
def _ensure_default_auth_user() -> None:
    ...
    try:
        ...
        default_user = models.User(
            ...
            display_name="系统管理员",
            email=None,
            password_hash=_build_password_hash(settings.ADMIN_PASSWORD),
            is_active=True,
            is_admin=True,
        )
        db.add(default_user)
...
async def _fetch_ozon_entrypoint_payloads_via_cookie(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=20.0, follow_redirects=True, trust_env=False) as client:
        for path in _ozon_entrypoint_paths(product_id):
            endpoint = (
...
async def _fetch_ozon_entrypoint_payloads(
    ...
    *,
    ...
    if use_browser_session:
        return _fetch_ozon_entrypoint_payloads_from_browser(
            product_id,
...
async def _run_cloud_follow_submit_workflow(
    *,
        ...
        if variant_ids:
            ...
            for variant_id in variant_ids:
                if int(variant_id) == resolved_product_id:
                    source_payloads.append(base_product_data)
                    continue
                try:
    ...
    for source_payload in source_payloads:
        try:
            prepared_item = await build_upload_item(
                client_id=store.client_id,
                api_key=store.api_key,
                scraped_data=source_payload,
                price=price,
...
def batch_update_product_price(
    ...
    db.commit()
    return {"message": f"Updated price for {len(products)} products"}


@app.post(f"{settings.API_V1_STR}/products/batch/stock")
def batch_update_product_stock(
...
def batch_archive_products(
    ...
    db.commit()
    return {"message": f"Updated archive status for {len(products)} products"}
...
def batch_remark_products(
    ...
    db.commit()
    return {"message": f"Updated remark for {len(products)} products"}
...
def batch_retry_products(
    ...
    for product in products:
        product.status = "approved"
    db.commit()
    return {"message": f"Retried {len(products)} products"}
