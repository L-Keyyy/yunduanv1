...
if ($BindAddress -and $BindAddress.Trim()) {
    $sshOptions += @("-b", $BindAddress.Trim())
}
...
'@
    ...
    try {
        ...
        Invoke-CheckedWithRetry -FilePath $sshPath -Arguments @(
            $sshOptions +
            $remoteTarget,
            "test ! -d $remoteStaging && sudo systemctl is-active ozon-backend ozon-worker ozon-upload-worker ozon-beat nginx >/dev/null && curl -fsS http://127.0.0.1:8000/healthz >/dev/null"
        )
    }
    ...
    if (-not $SkipVerify) {
        ...
        Invoke-CheckedWithRetry -FilePath $sshPath -Arguments @(
            $sshOptions +
            $remoteTarget,
            "set -e; curl -fsS -I http://127.0.0.1/register | head -n 5"
        )
    }
