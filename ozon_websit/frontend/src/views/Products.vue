...
const isUserCancel = (error: any) => error === 'cancel' || error === 'close'
...
const promptPrice = async (ids: number[], initialValue?: number) => {
  try {
    ...
    const data = await batchUpdateProductPrice({ ids, price: Number(value) })
    ElMessage.success(data.message || '改价完成')
    await handleSearch()
  } catch (error: any) {
    if (isUserCancel(error)) return
    ElMessage.error(getActionErrorMessage(error, '改价失败'))
  }
}
...
const promptStock = async (rows: any[], initialValue?: number) => {
  ...
  try {
    ...
    const data = await batchUpdateProductStock({
      ids: rows.map((row) => row.id),
      stock: Number(value),
      warehouse_name: warehouseName,
    })
    ElMessage.success(data.message || '库存更新完成')
    await handleSearch()
  } catch (error: any) {
    if (isUserCancel(error)) return
    ElMessage.error(getActionErrorMessage(error, '库存更新失败'))
  }
}
...
const promptRemark = async (ids: number[], initialValue = '') => {
  try {
    ...
    const data = await batchRemarkProducts({ ids, remark: value })
    ElMessage.success(data.message || '备注已更新')
    await handleSearch()
  } catch (error: any) {
    if (isUserCancel(error)) return
    ElMessage.error(getActionErrorMessage(error, '备注更新失败'))
  }
}
...
const handleBatchArchive = async (archived: boolean) => {
  if (!ensureSelected()) return
  try {
    const data = await batchArchiveProducts({ ids: selectedIds(), archived })
    ElMessage.success(data.message || (archived ? '已归档' : '已取消归档'))
    await handleSearch()
  } catch (error: any) {
    ElMessage.error(getActionErrorMessage(error, archived ? '归档失败' : '恢复失败'))
  }
}
...
const handleBatchRetry = async () => {
  if (!ensureSelected()) return
  try {
    const data = await batchRetryProducts({ ids: selectedIds() })
    ElMessage.success(data.message || '已重试选中商品')
    await handleSearch()
  } catch (error: any) {
...
const handleSingleArchive = async (row: any, archived: boolean) => {
  try {
    const data = await batchArchiveProducts({ ids: [row.id], archived })
    ElMessage.success(data.message || (archived ? '商品已归档' : '商品已恢复'))
    await handleSearch()
  } catch (error: any) {
    ElMessage.error(getActionErrorMessage(error, archived ? '归档失败' : '恢复失败'))
  }
...
const handleSingleRetry = async (row: any) => {
  try {
    const data = await batchRetryProducts({ ids: [row.id] })
    ElMessage.success(data.message || '商品已重试')
    await handleSearch()
  } catch (error: any) {
    ElMessage.error(getActionErrorMessage(error, '重试失败'))
  }
