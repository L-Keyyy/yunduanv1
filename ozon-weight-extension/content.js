...
function removeLegacyDashboardArtifacts(productId, host) {
  for (const panel of document.querySelectorAll(
  ...
  if (host) {
    for (const stale of host.querySelectorAll(
      `:scope > .${PRODUCT_DASHBOARD_SLOT_CLASS}[data-product-id]:not([data-product-id="${productId}"])`
    )) {
      stale.remove();
    }
  }
...
function getBuyerDashboardSlot(host, layout, productId) {
  ...
  let slot = host.querySelector(
    `:scope > .${PRODUCT_DASHBOARD_SLOT_CLASS}[data-product-id="${productId}"]`
  );
...
function buildCardDashboardMarkup(record, template, job, options = {}) {
  ...
  return `
    <div class="${PRODUCT_DASHBOARD_CLASS} notranslate" translate="no" data-product-id="${escapeHtml(productId)}" data-layout="card" data-product-url="${escapeHtml(
      options.productUrl || ""
    )}">
...
function buildProductDashboardMarkup(record, template, job, options = {}) {
  ...
  return `
    <div class="${PRODUCT_DASHBOARD_CLASS} notranslate" translate="no" data-product-id="${escapeHtml(productId)}" data-layout="${escapeHtml(
      layout
    )}" data-product-url="${escapeHtml(options.productUrl || "")}" data-product-weight="${escapeHtml(
