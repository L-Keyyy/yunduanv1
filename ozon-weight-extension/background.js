...
const HD_AUTH_COOKIE_NAMES = ["omtozon_token", "ozon_token"];
const HD_LOCAL_TOKEN_KEYS = ["ozon_access_token", "ozon_token", "access_token", "token"];
const HD_LOCAL_USER_KEYS = ["ozon_auth_user", "ozon_user", "auth_user", "user"];
const HD_ME_PATHS = ["/api/v1/me", "/api/v1/auth/me"];
...
function getOriginFromUrl(url, fallback = HD_PRIMARY_APP_ORIGIN) {
  try {
...
function unwrapHdUser(rawUser) {
  if (!rawUser || typeof rawUser !== "object") {
    return {};
  }
  if (rawUser.user && typeof rawUser.user === "object") {
    return rawUser.user;
  }
  return rawUser;
}
...
function normalizeHdSession(token, rawUser, origin = HD_PRIMARY_APP_ORIGIN) {
  ...
  const user = unwrapHdUser(rawUser);
  const userId = user.id ?? user.user_id ?? jwtPayload.uid ?? jwtPayload.userId ?? jwtPayload.user_id ?? null;
  ...
  const tenantId = user.tenant_id ?? user.tenantId ?? jwtPayload.tenant_id ?? jwtPayload.tenantId ?? null;
  const tenantName = user.tenant_name || user.tenantName || null;
  const displayName = user.display_name || user.displayName || user.name || username || "SaaS 用户";
...
async function fetchHdUserFromToken(token, origin = HD_PRIMARY_APP_ORIGIN) {
  if (!token) {
    return null;
  }

  for (const pathname of HD_ME_PATHS) {
    try {
      ...
      return unwrapHdUser(data);
    } catch (_error) {
      // Keep trying the next supported account endpoint.
    }
  }
...
async function buildHdSessionFromToken(token, rawUser, origin = HD_PRIMARY_APP_ORIGIN, options = {}) {
  if (!token) {
    return null;
  }

  const normalizedOrigin = origin || HD_PRIMARY_APP_ORIGIN;
  const shouldVerify = Boolean(options.verify);
  const localUser = unwrapHdUser(rawUser);
  let user = localUser;
  let session = normalizeHdSession(token, user, normalizedOrigin);

  if (shouldVerify || !session || !Object.keys(user).length) {
    const fetchedUser = await fetchHdUserFromToken(token, normalizedOrigin);
    if (fetchedUser && Object.keys(fetchedUser).length) {
      user = { ...user, ...fetchedUser };
      session = normalizeHdSession(token, user, normalizedOrigin);
...
async function readHdSessionFromDashboardTabs() {
  ...
  for (const tab of sortedTabs) {
    try {
      const results = await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: (tokenKeys, userKeys) => {
          const readFirstLocalStorageValue = (keys) => {
          ...
          const tokenEntry = readFirstLocalStorageValue(tokenKeys);
          let user = null;
          let userKey = null;
        ...
        args: [HD_LOCAL_TOKEN_KEYS, HD_LOCAL_USER_KEYS]
      });
      const payload = results?.[0]?.result || null;
      const token = payload?.token || null;
      const origin = payload?.origin || getOriginFromUrl(tab.url);
      if (token) {
        const session = await buildHdSessionFromToken(token, payload?.user, origin);
        if (session) {
          return session;
        }
      }
...
async function readHdAuthCookieToken(origin = HD_PRIMARY_APP_ORIGIN) {
  if (!chrome.cookies?.get) {
    return null;
  }

  for (const cookieName of HD_AUTH_COOKIE_NAMES) {
    const cookie = await chrome.cookies.get({ url: origin, name: cookieName }).catch(() => null);
    if (cookie?.value) {
      return cookie.value;
    }
  }
...
async function readHdSessionFromAuthCookies() {
  for (const origin of HD_API_ORIGINS) {
    const token = await readHdAuthCookieToken(origin);
    if (!token) {
      continue;
    }
    const session = await buildHdSessionFromToken(token, null, origin, { verify: true });
    if (session) {
      return session;
    }
  }
...
async function getStoredHdSession(verify = false) {
  ...
  const storedSession = stored[HD_AUTH_SESSION_STORAGE_KEY] || null;
  if (!token) {
    return null;
  }

  if (verify) {
    const preferredOrigin = storedSession?.origin || HD_PRIMARY_APP_ORIGIN;
    const origins = [
      preferredOrigin,
      ...HD_API_ORIGINS.filter((origin) => origin !== preferredOrigin)
    ];
    for (const origin of origins) {
      const verifiedSession = await buildHdSessionFromToken(token, storedSession?.user || null, origin, {
        verify: true
      });
      if (verifiedSession) {
  ...
  const session = storedSession || normalizeHdSession(token, null);
  if (!session) {
    return null;
  }
  return session;
...
async function getHdSession(forceRefresh = false) {
  if (forceRefresh) {
    ...
    const cookieSession = await readHdSessionFromAuthCookies();
    if (cookieSession) {
      return cookieSession;
    }
  }

  const storedSession = await getStoredHdSession(forceRefresh);
  if (storedSession) {
    return storedSession;
  }

  return (await readHdSessionFromDashboardTabs()) || readHdSessionFromAuthCookies();
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    ...
    if (message.type === "connect-hd-account") {
      const session = await getHdSession(true);
      if (session) {
        sendResponse({ ok: true, session });
        return;
      }
