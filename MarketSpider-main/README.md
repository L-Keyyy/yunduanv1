...
淘宝、京东、1688商品信息爬虫。方便自动化的获取指定关键词的商品链接、商品价格、商品名称、店铺名称、店铺链接等信息。配合浏览器版控制台，可以清晰配置和启动各项功能。  
...
#### 4、图形启动器
本目录新增 `MarketSpider_WebUI.py` 浏览器控制台，可直接配置浏览器、输出格式、检查依赖、获取 Cookie、启动淘宝/京东/1688 爬虫和更新工具。它不依赖 Tkinter，可避开 macOS 上 Tk 窗口全白的问题。

- macOS：双击 `StartMarketSpiderUI.command`
- Windows：双击 `StartMarketSpiderUI.bat`
- 手动启动：在本目录运行 `python3 MarketSpider_WebUI.py`

如果运行状态提示缺少依赖，先点击 UI 里的“安装/修复依赖”，安装完成后回到 UI 点击“检查状态”。
