...
def app_file(dirname: str, filename: str):
    ensure_dir(dirname)
    return os.path.join(dirname, filename)


def compatible_file(dirname: str, filename: str):
    normal_path = os.path.join(dirname, filename)
    legacy_path = f"{dirname}\\{filename}"
...
class Logger:
    def __init__(self, comp: str):
        ...
        self.log_file = open(app_file("logs", f"{log_file_name}.log"), "a", encoding="utf8")
...
class BrowserControl:
    ...
    def inject_cookie(self, market):
        ...
        try:
            ...
            with open(compatible_file("cookie", f"{market}.cookie"), "r") as f:
            ...
            with open(compatible_file("cookie", f"{market}.storage"), "r") as f:
                list = json.load(f)
    ...
    def save_cookie(self, market) -> bool:
        try:
            ensure_dir("cookie")
            with open(app_file("cookie", f"{market}.cookie"), "w") as file:
                file.write(json.dumps(self.browser.get_cookies()))
            with open(app_file("cookie", f"{market}.storage"), "w") as file:
                storage = {}
...
class CsvWriter:
    def __init__(self, keyword, market):
        ...
        ensure_dir("result")
        self.csvStream = open(
            app_file("result", f'{keyword}-{market}-{time.strftime("%Y-%m-%d_%H-%M", time.localtime())}.csv'),
            "a",
            encoding="utf-8-sig",
            newline="",
        )
...
def pre_check_cookie(market):
    try:
        f = open(compatible_file("cookie", f"{market}.cookie"), "r")
        f.close()
        return False
