...
import {
  ...
  syncOzonCategoryAttributeTranslations,
...
const syncSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("category_attribute_translations_zh"),
    categoryRecordIds: z.array(z.string().min(1)).min(1).max(50),
  }),
]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (parsed.action === "category_attribute_translations_zh") {
      const result = await syncOzonCategoryAttributeTranslations(
        parsed.categoryRecordIds,
      );
      return ok(result);
    }
