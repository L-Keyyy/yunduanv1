...
import { ChevronsUpDown, CopyPlus, History, Loader2, PlugZap, Plus, Save, Search } from "lucide-react";
import { z } from "zod";
...
export function ProviderSettings({ initialProviders }: ProviderSettingsProps) {
  ...
  const selectedProvider = useMemo(
    () => (selectedProviderId ? providers.find((item) => item.id === selectedProviderId) ?? null : null),
  ...
  function handleNewProvider() {
    ...
    setModels([]);
    setDefaults(buildDefaults(null));
    reset({
      id: undefined,
      name: "",
      baseUrl: "",
    ...
    toast.info("已切到新增 API 配置，请填写新的 baseURL 和 API Key");
  }
	          ...
	          <div className="space-y-3 rounded-3xl border border-border bg-muted/40 p-4">
	            <div className="flex flex-wrap items-center justify-between gap-3">
	              <div className="flex items-center gap-2">
	                ...
	                <h3 className="font-medium">已保存 API 服务</h3>
	                <Badge variant="outline">{providers.length} 个</Badge>
	              </div>
	              <Button type="button" variant="outline" size="sm" onClick={handleNewProvider} disabled={loading !== null}>
	            ...
	            <div className="flex flex-col gap-3 md:flex-row">
	              <div className="flex-1 space-y-2">
	                <Label htmlFor="provider-history">选择已保存服务</Label>
                    ...
                    className="flex h-10 w-full appearance-none rounded-xl border border-input bg-background px-3 pr-10 text-sm text-foreground dark:bg-black/30"
	                  ...
	                  >
	                    <option value="">{providers.length === 0 ? "暂无已保存服务，先新增一个" : "新增配置 / 不读取历史服务"}</option>
                ...
                <Button
                  ...
                  onClick={() => selectedProviderId && handleActivateProvider(selectedProviderId)}
	                ...
	                >
	                  ...
	                  {selectedProvider?.isActive ? "默认服务" : "设为默认服务"}
	                </Button>
              </div>
            </div>
            {selectedProvider ? (
	              <div className="rounded-2xl border border-border bg-background p-4 dark:bg-black/20">
	                <div className="flex flex-wrap items-center gap-2">
	                  ...
	                  {selectedProvider.isActive ? <Badge variant="success">默认服务</Badge> : null}
	                  <Badge variant="outline">{selectedProvider.models.length} 个模型</Badge>
	                </div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">最近更新：{formatTimeLabel(selectedProvider.updatedAt)}</p>
	              ...
	              <div className="rounded-2xl border border-dashed border-border p-4 text-sm text-muted-foreground">
	                正在新增 API 配置。保存后，这里可以继续新增 DeepSeek、OpenAI、Gemini 网关或自建兼容服务。
	              </div>
            )}
          </div>
          ...
          <form autoComplete="off" className="grid gap-4 md:grid-cols-2" onSubmit={(event) => event.preventDefault()}>
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <Input
                ...
                data-lpignore="true"
                data-form-type="other"
                placeholder="OpenAI 兼容：https://your-provider.example/v1；Gemini 官方：https://generativelanguage.googleapis.com"
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <p className="text-xs text-muted-foreground">
                选择历史服务后，名称和 URL 会回填；Key 留空时会复用已保存密钥，输入新 Key 才会覆盖。
              </p>
            </div>
          </form>
