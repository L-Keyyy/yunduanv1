...
async function upsertOzonAttributeRecord(params: { categoryId: string; record: UnknownRecord; now?: Date }) {
  ...
  return prisma.ozonAttribute.upsert({
    where: {
...
async function upsertOzonAttributeValueRecord(params: { attributeId: string; record: UnknownRecord; now?: Date }) {
    ...
    if (includeAttributes) {
      ...
      for (const file of attributeFiles) {
        ...
        for (const attr of attrs) {
          ...
          const syncedAttribute = await upsertOzonAttributeRecord({ categoryId: category.id, record, now });
...
async function syncAttributeValues(params: {
  ...
  while (hasNext && synced < params.maxValues) {
    ...
    for (const value of values) {
      ...
      const didSync = await upsertOzonAttributeValueRecord({ attributeId: params.categoryId, record, now });
      if (!didSync) continue;
