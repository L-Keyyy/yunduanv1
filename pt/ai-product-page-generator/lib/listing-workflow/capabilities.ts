...
export async function scanListingWorkflowCapabilities(): Promise<ListingWorkflowCapabilities> {
  ...
  let ocrServiceReady = ocrFilesReady ? await endpointReady(ocrEndpoint) : false;
  if (ocrFilesReady && !ocrServiceReady) {
    try {
      await ensureDoubaoWebService();
      ocrServiceReady = await endpointReady(ocrEndpoint);
    } catch {
    ...
    capabilities: [
      ...
      },
      {
        ...
        summary: ocrServiceReady
          ? "图片翻译服务在线，可选择 Google 或豆包网页版识别并翻译回填。"
          : ocrFilesReady
