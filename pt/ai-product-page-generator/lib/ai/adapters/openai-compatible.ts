...
function deriveGoogleBaseUrl(baseUrl: string) {
  const normalized = normalizeBaseUrl(baseUrl);
  const withoutVersion = normalized.replace(/\/v1(?:beta)?$/i, "");

  if (/generativelanguage\.googleapis\.com/i.test(withoutVersion)) {
...
function isGoogleGeminiNativeBaseUrl(baseUrl: string) {
  return /generativelanguage\.googleapis\.com/i.test(normalizeBaseUrl(baseUrl));
}
...
function extractGoogleTextContent(payload: unknown) {
  const parts = (payload as { candidates?: Array<{ content?: { parts?: Array<Record<string, unknown>> } }> })
...
function googleModelModalities(model: Record<string, unknown>) {
  const name = String(model.name ?? "");
  const methods = Array.isArray(model.supportedGenerationMethods)
    ? model.supportedGenerationMethods.filter((entry): entry is string => typeof entry === "string")
    : [];
  const id = normalizeGoogleModelName(name).toLowerCase();

  if (/imagen|image|banana/.test(id)) {
    return ["image_generation", ...methods];
  }

  return ["text", "vision", ...methods];
}
...
function buildGoogleGenerateContentPayload(input: {
  systemPrompt?: string;
  userPrompt: string;
  images?: string[];
  temperature?: number;
  ...
  for (const image of input.images ?? []) {
    if (/^data:.+;base64,/.test(image)) {
      parts.push({ inlineData: dataUrlToInlineData(image) });
    } else {
...
export class OpenAICompatibleAdapter implements ProviderAdapter {
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          ...(!extraHeaders?.["x-goog-api-key"] ? { Authorization: `Bearer ${this.apiKey}` } : {}),
          ...(extraHeaders ?? {}),
          ...(init?.headers ?? {}),
        },
        cache: "no-store",
  ...
  private async requestGoogleGetJson<T>(path: string, timeoutMs = 15000, monitor?: AiMonitorContext) {
    ...
    for (const url of attempts) {
      ...
      if (!shouldRetryWithVersionedBase(response.status, response.body)) {
        break;
      }
    }

    throw new Error(`Google protocol request failed (${lastStatus}): ${lastBody}`);
  }
  ...
  async testConnection() {
    if (isGoogleGeminiNativeBaseUrl(this.baseUrl)) {
      await this.listGoogleModels();
      return {
        ok: true,
  ...
  private async listGoogleModels() {
    const payload = await this.requestGoogleGetJson<{
    ...
    return (payload.models ?? [])
      ...
      .map((item) => {
        const id = normalizeGoogleModelName(String(item.name));
        return {
          id,
          label: String(item.displayName ?? id),
          type: /imagen|image|banana/i.test(id) ? "image" : "text",
          category: /imagen|image|banana/i.test(id) ? "image" : "text",
          modalities: googleModelModalities(item),
        };
      });
  }
  ...
  async generateText(input: TextRequest) {
    if (isGoogleGeminiNativeBaseUrl(this.baseUrl)) {
      const payload = await this.requestGoogleJson<any>(`/models/${input.model}:generateContent`, buildGoogleGenerateContentPayload({
      ...
      return {
        text: extractGoogleTextContent(payload),
      };
    }
  ...
  async generateStructured<T>(input: StructuredRequest<T>) {
    if (isGoogleGeminiNativeBaseUrl(this.baseUrl)) {
      ...
      const raw = extractGoogleTextContent(payload);

      try {
        const parsed = tryParseStructuredPayload(raw, input.schema);
        return { parsed, raw };
      } catch (error) {
        return this.repairGoogleStructuredOutput(
          input,
          raw,
  ...
  private async repairGoogleStructuredOutput<T>(input: StructuredRequest<T>, raw: string, reason: string) {
    const payload = await this.requestGoogleJson<any>(`/models/${input.model}:generateContent`, buildGoogleGenerateContentPayload({
    ...
    const repairedRaw = extractGoogleTextContent(payload);
    return {
      parsed: tryParseStructuredPayload(repairedRaw, input.schema),
      raw: repairedRaw,
    };
