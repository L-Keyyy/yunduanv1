...
export async function resolveProviderConnectionInput(
  ...
  if (!matchedProvider) {
    throw new Error("当前没有可复用的已保存 API Key，请输入 API Key 后再试。");
  }
...
export async function getProviderAdapter(providerId?: string): Promise<ProviderAdapterContext> {
  ...
  const apiKey = decryptSecret(provider.apiKeyEncrypted);
  const runtimeModels = mergeHydratedProviderModels(
    provider.id,
    provider.baseUrl,
    hydrateProviderModels(provider.models) as unknown as RuntimeProviderModel[],
  );
