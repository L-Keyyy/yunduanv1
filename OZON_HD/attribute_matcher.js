...
const COLOR_FALLBACK_VALUES = ['Разноцветный', 'Мультиколор', 'Многоцветный', 'Цветной', 'Другой'];

function isColorAttribute(attr) {
  const normalizedName = norm(attr?.name);
  return normalizedName.includes('цвет') || normalizedName.includes('color') || normalizedName.includes('colour');
}

function normalizeColorValue(rawValue) {
  const text = String(rawValue || '').trim();
  if (!text || ['-', '--', 'n/a', 'na', 'none', 'нет'].includes(norm(text))) {
    return '';
  }
  return text;
}

function expandColorCandidates(rawValue) {
  const colorText = normalizeColorValue(rawValue);
  if (!colorText) {
    return [];
  }

  const candidates = [colorText];
  const normalized = norm(colorText);
  for (const [needles, aliases] of COLOR_ALIAS_GROUPS) {
...
function findColorDictValue(dictValues, textValue) {
  for (const candidate of expandColorCandidates(textValue)) {
    const matched = findDictValue(dictValues, candidate);
    if (matched) {
      return matched;
    }
  }
  for (const fallback of COLOR_FALLBACK_VALUES) {
    const matched = findDictValue(dictValues, fallback);
    if (matched) {
      return matched;
    }
  }
  ...
  for (const char of scrapedChars) {
    ...
    for (const val of values) {
      if (ozonAttr.dictionary_id && ozonAttr.dictionary_id > 0) {
        ...
        const colorAttr = isColorAttribute(ozonAttr);
        const dictMatch = colorAttr ? findColorDictValue(dictValues, val) : findDictValue(dictValues, val);
        if (dictMatch) {
          ozonValues.push({
          ...
          // 颜色属性是强字典项，不能提交 dictionary_value_id=0 的自由文本。
          continue;
        } else {
          // 字典中未找到，尝试作为自定义值提交
          ozonValues.push({
...
module.exports = { matchAttributes, buildAttributeNameMap, findDictValue, findColorDictValue, isColorAttribute };
