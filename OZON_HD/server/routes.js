...
function isOmtHost(hostname) {
  return /(^|\.)omtozon\.com$/i.test(String(hostname || ''));
}
...
function authCookieOptions(req) {
  ...
  const secure =
    process.env.AUTH_COOKIE_SECURE === undefined
      ? req.secure || forwardedProto === 'https' || isOmtHost(hostname)
  ...
  const domain =
    process.env.AUTH_COOKIE_DOMAIN === undefined
      ? isOmtHost(hostname) ? '.omtozon.com' : undefined
      : process.env.AUTH_COOKIE_DOMAIN || undefined;
...
function resolveExtensionUser(req) {
  try {
    ...
    // Return the explicit commercial-mode error below.
  }

  if (process.env.ALLOW_LOCAL_EXTENSION_FALLBACK !== '1') {
    throw new Error('Please log in to 35.209.87.105 before using the extension.');
  }
